...
def save(msg_sent):
    sql = """
        ...
        RETURNING *
    """
    #i dont insert a value ie '%s' for the id value here, because it hasnt been generated yet - because its a new item, and thereore doesnt have an id yet. Thats why its listed as None in the class construtor file
    ...
    if results:
        ...
        id = results[0]['id']  # the 'id' is in the first row of results
        msg_sent.id = id
        return msg_sent
    else:
        # if there were no results returned, essentially error handling
        return None